...
class ModelInspector(object):
    ...
    def build_model(self, inputs, is_training=False):
    ...
    def saved_model_benchmark(self, image_path_pattern, **kwargs):
        ...
        raw_images = [
            np.array(Image.open(f)) for f in all_files[:self.batch_size]
        ]
    ...
    def saved_model_video(self, video_path, output_video, **kwargs):
        ...
        if output_video:
            ...
            out_ptr = cv2.VideoWriter(
                output_video, cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), 25,
    ...
    def run_model(self, runmode, threads=0):
        ...
        if runmode == 'dry':
            ...
            if runmode == 'infer':
                self.inference_single_image(FLAGS.input_image,
                                            FLAGS.output_image_dir,
                                            **config_dict)
            elif runmode == 'saved_model':
                self.export_saved_model(**config_dict)
            elif runmode == 'saved_model_infer':
                self.saved_model_inference(FLAGS.input_image,
                                           FLAGS.output_image_dir,
                                           **config_dict)
            elif runmode == 'saved_model_video':
                self.saved_model_video(FLAGS.input_video, FLAGS.output_video,
                                       **config_dict)
        elif runmode == 'bm':
            self.benchmark_model(warmup_runs=5,
...
def main(_):
    ...
    inspector = ModelInspector(model_name=FLAGS.model_name,
                               logdir=FLAGS.logdir,
