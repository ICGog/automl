...
def get_ckpt_var_map(ckpt_path, ckpt_scope, var_scope, var_exclude_expr=None):
  ...
  """
    ...
    exclude_matcher = re.compile(
        var_exclude_expr) if var_exclude_expr else None
    for v in model_vars:
        if exclude_matcher and exclude_matcher.match(v.op.name):
            logging.info('skip {} -- excluded by {}'.format(
                v.op.name, var_exclude_expr))
            continue
        ...
        if ckpt_var not in ckpt_var_names:
            if v.op.name.endswith('/ExponentialMovingAverage'):
                ckpt_var = ckpt_scope + v.op.name[:-len(
...
def batch_norm_act(inputs,
  ...
  """
    ...
    inputs = tpu_batch_normalization(inputs=inputs,
                                     axis=axis,
                                     momentum=momentum,
                                     epsilon=epsilon,
                                     center=True,
